...
## React Frontend (Bootstrap + fetch)

A corresponding React frontend is added in `frontend/`.
...
The frontend calls API at: `http://localhost:5227/api` (configured in `frontend/src/api.js`).

### Frontend features

- Login with JWT token storage in LocalStorage
- Protected routes by authentication and role (`Admin` / `User`)
- User dashboard:
